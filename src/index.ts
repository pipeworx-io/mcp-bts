...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'U.S');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'dataset_columns': {
      ...
      return getJson(`${PORTAL}/api/views/${encodeURIComponent(id)}.json`, id);
    }
    case 'query_dataset': {
      const id = reqStr(args, 'datasetId', '"crem-w557"');
      ...
      params.set('$offset', String(numOr(args.$offset, 0)));
      return getJson(`${PORTAL}/resource/${encodeURIComponent(id)}.json?${params.toString()}`, id);
    }
    default:
...
async function getJson(url: string, datasetId?: string): Promise<unknown> {
  const res = await pwFetch(url, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  // A 404 on a caller-supplied 4x4 id is that id not existing on data.bts.gov,
  // not an outage. Every live failure in the 2026-08 window was an id from a
  // DIFFERENT Socrata portal (jbjy-vk9h / p6dx-8zbt are datos.gov.co SECOP
  // datasets) — 4x4 codes are portal-scoped and never transfer.
  if (res.status === 404 && datasetId) {
    throw new Error(`user_error: BTS: no dataset "${datasetId}" exists on data.bts.gov. Socrata 4x4 ids are scoped to one portal — an id from another Socrata site will not resolve here (for Colombia's datos.gov.co ids like SECOP, use datos_co_query_dataset instead). Use the BTS dataset search tool to find a data.bts.gov id (e.g. "crem-w557").`);
  }
